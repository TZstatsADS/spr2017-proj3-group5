...
```{r}
img_train$V1<-factor(img_train$V1)
img_test$V1<-factor(img_test$V1)
svm_siftl <- vector(length=5)
for(i in 3:6){
      SVM_Temp<- svm(V1~., data=img_train, kernel="linear",cost=3^i,scale=FALSE)
      svm_predictl<-predict(SVM_Temp,img_test[,-5001])
      svm_siftl[i] <-mean(svm_predictl!=img_test[,5001])
  }
svm_siftl2 <- vector(length=5)
for(i in 2:6){
      SVM_Temp<- svm(V1~., data=img_train, kernel="linear",cost=3^i,scale=TRUE)
      svm_predictl<-predict(SVM_Temp,img_test[,-5001])
      svm_siftl2[i] <-mean(svm_predictl!=img_test[,5001])
  }  
...
svm_siftk<-vector(length=4)
for(i in 3:6){
      SVM_Temp<- svm(V1~., data=img_train, kernel="radial",cost=3^i,gamma=3^i,scale=FALSE)
      svm_predictl<-predict(SVM_Temp,img_test[,-5001])
      svm_siftk[i] <-mean(svm_predictl!=img_test[,5001])
}
svm_siftk<-vector(length=16)
for(i in 2:5){
    for(j in 2:5){
        SVM_Temp<- svm(V1~., data=img_train, kernel="radial",cost=3^i,gamma=3^j,scale=FALSE)
+       svm_predictl<-predict(SVM_Temp,img_test[,-5001])
+       svm_siftk[4*(i-2)+j-1] <-mean(svm_predictl!=img_test[,5001])
      }
}
```
