...
% trainingFeatures   = zeros(trainingSet(1).Count,hogFeatureSize,'single');  
trainingFeatures=[];
trainingLabels=[];
numImages=trainingSet(1).Count;
features=zeros(numImages,hogFeatureSize,'single');
    for i = 1:numImages
        %img=rgb2gray(read(trainingSet(1),i));
%         img = readImage(trainingSet(1), i);
