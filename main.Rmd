...
fit_gbm233<-gbm.fit(x=img_train[,1:1999],y=img_train[,2000],n.trees = 1999,distribution = "bernoulli",interaction.depth = 10,bag.fraction = 0.5,verbose = FALSE)
#img_train$V1 <- factor(img_train$V1)
#gbm1 <- gbm(formula = V1~., data = img_train, n.trees = 500)
#exp(predict(fit_gbm, img_train[,-2000], n.tree = 1000))
...
#hog_1999+svm_kernel
```{r}
tuneSVMkernel<-tune(svm,V1~.,data = img_train,kernel="radial",ranges = list(cost=c(0.5,1,2,5,10,20),gamma=c(1,5,10,20),tunecontrol = tc,scale=FALSE))
best_kernel<-tuneSVMkernel$best.model
kernel_predict<-ifelse(predict(best_kernel,img_test[,-2000])>0.5,1,0)
